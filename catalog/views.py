...
def output_order(results):#对时间排序
    sorted_objects = results.order_by('-created_at')
...
@login_required
def delete_remark(request,id):  #删除贴子视图
    post = get_object_or_404(Remark,name=request.user,pk=id)
    #尝试获取一个Remark
    post.delete()
    results=request.user.input_infos.all()
    results = output_order(results)
    return render(request,'mine_remark.html',{'results':results})  # 重定向到其他视图，例如列表视图

from .forms import RemarkForm_
from django.db import models
from django.utils import timezone
@login_required
def change_remark(request,id):   #修改视图
    remark, created = Remark.objects.get_or_create(name=request.user,pk=id)
    if request.method == 'POST':
        form = RemarkForm_(request.POST, instance=remark)
        if form.is_valid():
            input_info = form.save(commit=False)
            input_info.created_at = timezone.now() #更新时间
            input_info.save()
            results=request.user.input_infos.all()
            results=output_order(results)
            return render(request,'mine_remark.html',{'results':results}) # 重定向到我的贴子视图
    else:
        form = RemarkForm_(instance=remark) #得到要修改的某个具体对象
